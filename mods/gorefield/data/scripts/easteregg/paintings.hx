...
function create() {
    ...
    if (FlxG.save.data.arlenePhase < 0) return;
    if (FlxG.save.data.paintPosition == -1) return;
    if (!positionMap.exists(stageName)) return;

    var stagePositionMap = positionMap.get(stageName);
    if (!stagePositionMap.exists(FlxG.save.data.paintPosition)) return;
