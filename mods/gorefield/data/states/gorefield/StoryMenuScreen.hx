...
var ArleneHint:FlxText;
var ArleneHint:FunkinText;
...
function create() {
	...
	#if mobile
	FlxG.stage.addEventListener(TextEvent.TEXT_INPUT, onTextInputMobile);
	FlxG.stage.addEventListener(KeyboardEvent.KEY_DOWN, onKeyDownMobile);
	#end
	...
	ArleneHint = new FunkinText(-240, 611, FlxG.width, "Tap the C button to visit Arlene", 24, true);
	ArleneHint.setFormat("fonts/pixelart.ttf", 54, FlxColor.WHITE, "left", FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	ArleneHint.borderSize = 4;
	ArleneHint.scrollFactor.set();
	ArleneHint.scale.set(0.4, 0.4);
	ArleneHint.cameras = [camText];
	ArleneHint.alpha = 0.7;
	ArleneHint.visible = FlxG.save.data.canVisitArlene;
	add(ArleneHint);
	FlxTween.tween(ArleneHint, {alpha: 0}, 1.2, {
	...
	overlayBlack = new FlxSprite(0, 0);
    overlayBlack.makeGraphic(FlxG.width, FlxG.height, FlxColor.BLACK);
    overlayBlack.scrollFactor.set(0, 0);
    overlayBlack.cameras = [camText];
    overlayBlack.active = false;
    overlayBlack.visible = false;
    add(overlayBlack);
}
...
function update(elapsed:Float) {
    ...
    if ((FlxG.keys.justPressed.TAB || touchPad.buttonC.justPressed) && canMove && !codesOpened && FlxG.save.data.canVisitArlene) {
        canMove = false;
        codesMenu(false, -100);

        FlxTween.tween(selector, {angle: 45, alpha: 0}, 0.8, {ease: FlxEase.circOut});
        ...
        if (ArleneHint != null) {
            FlxTween.cancelTweensOf(ArleneHint);
            ArleneHint.alpha = 0;
	...
	freeplayMenuText.x = 1060;
	freeplayMenuText.y = 100;
...
function openFreePlayMenu() {
    ...
    // Make ArleneHint disappear when opening Freeplay Menu
    if (ArleneHint != null) {
        FlxTween.cancelTweensOf(ArleneHint);
        ArleneHint.alpha = 0;
    }
...
function codesMenu(open:Bool, offset:Float) {
    ...
    codesTween = FlxTween.tween(codesPanel, {x: (codesOpened ? 0 : -415) + offset}, 0.25, {ease: FlxEase.circInOut});
    ...
    openCodesList(false, codesOpened ? false : true, previousOpen != open);
    ...
    if (ArleneHint != null) {
        if (open) {
            FlxTween.cancelTweensOf(ArleneHint);
            ArleneHint.visible = false;
        } else {
            ArleneHint.visible = true;
            ArleneHint.alpha = 0.7;
            FlxTween.tween(ArleneHint, {alpha:0}, 1.2, {
                type: FlxTween.PINGPONG,
                ease: FlxEase.quadInOut
...
function onKeyDown(keyCode:Int, modifier:Int) {
	...
	switch(keyCode) {
		...
		case V:
			// paste
			if (modifier == LEFT_CTRL || modifier == RIGHT_CTRL) // Esto a mi no me funciona :sob: -Aburridow
			{
...
function onKeyDownMobile(e:KeyboardEvent):Void {
    // ignore input if field is not focused
    if (!codesFocused || !canMove) return;

    // Backspace
    if (e.keyCode == 8) { // Backspace
        if (codesPosition > 0) {
            codesText.text = codesText.text.substr(0, codesPosition - 1) +
                             codesText.text.substr(codesPosition);
            codesPosition = FlxMath.bound(codesPosition - 1, 0, codesText.text.length);
            carcetTime = 0;
        }
    }
...
function onTextInputMobile(e:TextEvent):Void {
    codesText.text += e.text;
}

var curVideoMeme:FlxVideoSprite;
var CodesFunctions:{} = {
	...
	image: function(path:String) {
		...
		var newSprite:FlxSprite =
			new FlxSprite().loadGraphic(
				Paths.image("menus/storymenu/wow i love easter what about you/" + path)
			);

		newSprite.updateHitbox();
		newSprite.antialiasing = true;
		newSprite.alpha = 0;
		newSprite.angle = FlxG.random.float(-900, 900);
		newSprite.setPosition(
			FlxG.random.float(0, FlxG.width - newSprite.width),
			FlxG.random.float(0, FlxG.height - newSprite.height)
		);
		...
		FlxTween.tween(
			newSprite,
			{ "scale.x": 1, "scale.y": 1, alpha: 1, angle: 0 },
			0.3,
			{ ease: FlxEase.quadInOut }
		);
		...
		if (videos[curVideo] != null && videos[curVideo].bitmap.isPlaying) {
			videos[curVideo].togglePaused();
			videoWasPaused = !videoWasPaused;
			pauseIcon.visible = videoWasPaused;
		}
	...
	meme: function(path:String) {
		...
		if (video.load(Paths.video(path))) {
...
var codes:Map<String, Void -> Void> = [
	...
	"PENKARU" => function() CodesFunctions.penk(),
	...
	"ESTOY" => function() CodesFunctions.estoyCodeWarning(),
	...
	// Cheat codes
	"FULLCAT" => function() CodesFunctions.unlockAll(),
	"RESET" => function() CodesFunctions.resetAll(),
	"CATBOT" => function() CodesFunctions.catbot(),

	// Extras codes
	"SPIDERS" => function() CodesFunctions.meme("cry"),
	"MOUSTACHE" => function() {
		FlxG.switchState(new ModState("gorefield/CreditsScreen"));
		moustacheMode = true;
	},
...
function generateSECRET():Void {
	// Gather folder names under "songs" and normalize to lowercase
	var songsList:Array<String> = [];
	for (folder in Paths.getFolderDirectories('songs', false, false)) {
		var name:String = Std.string(folder);
		// If the returned value is a path, take the last component
		var parts:Array<String> = name.split("/");
		if (parts.length == 1) parts = name.split("\\");
		var basename:String = parts[parts.length - 1].toLowerCase();
		if (basename.length > 0) songsList.push(basename);
	}

	// If no save data, abort
	if (FunkinSave == null || FunkinSave.highscores == null) return;

	// Collect highscores that actually exist (we query the saved highscores directly)
	var highScores:Array<{name:String, score:Float}> = [];

	for (songName in songsList) {
		// Try to get the "hard" highscore for this song; skip if none
		var hs = FunkinSave.getSongHighscore(songName, "hard");
		if (hs == null) continue;

		// Safely extract an accuracy value if present
		var acc:Float = 0.0;
		var accField = Reflect.field(hs, "accuracy");
		if (accField != null) {
			// Convert to Float defensively (works if it's Float or numeric string)
			try {
				acc = Std.parseFloat(Std.string(accField));
			} catch (e:Dynamic) {
				acc = 0.0;
			}
		} else {
			// If there's no accuracy field, try other common fields (optional)
			var scoreField = Reflect.field(hs, "score");
			if (scoreField != null) {
				// We don't know the scale, but store something so it can be sorted
				try {
					acc = Std.parseFloat(Std.string(scoreField)) / 100.0;
				} catch (e:Dynamic) {
					acc = 0.0;
				}
			} else continue;
		...
		highScores.push({ name: songName, score: acc });
	}
	...
	// Use top score's accuracy and convert to percent
	var percent:Float = highScores[0].score * 100.0;

	// Build minute string with leading zero and take first digit
	...
	try {
		firstMinute = Std.parseInt(stringMinutes.charAt(0));
	} catch (e:Dynamic) {
		firstMinute = 0;
	}

	// Compose code digits (same logic as original)
	var codes:Array<Int> = [
		date.getDate() % 10,
		Std.int(Math.floor(percent)) % 10,
		3,
		1 + firstMinute
	];

	// Join into string (assumes secretCode is a global var)
