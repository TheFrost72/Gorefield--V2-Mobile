...
function update(elapsed:Float) {
    ...
    if ((FlxG.keys.justPressed.ENTER || FlxG.keys.justPressed.SPACE) && !pressedEnter && (skipLoadingAllowed || finishedLoading)) {
        goToSong();
        pressedEnter = true;
    }

    // Mobile input (touch) behaves exactly like ENTER/SPACE
    for (touch in FlxG.touches.list) {
        if (touch.justPressed && !pressedEnter && (skipLoadingAllowed || finishedLoading)) {
            goToSong();
            pressedEnter = true;
        }
    }
