...
{
    // Camera smooth movement using lerp
    var lerpVal:Float = Math.max(0, Math.min(1, elapsed * 7.5));
    camFollowPos.setPosition(
        FlxMath.lerp(camFollowPos.x, camFollow.x + camFollowXOffset, lerpVal),
    ...
    if (!quitting)
    {
        ...
        if (controls.UP_P) changeSelection(-3);
        if (controls.DOWN_P) changeSelection(3);

        // Return to main menu
        if (controls.BACK)
        {
            var sound:FlxSound = new FlxSound().loadEmbedded(Paths.sound("menu/cancelMenu"));
            sound.volume = 1;
            sound.play();
