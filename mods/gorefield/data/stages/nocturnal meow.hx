...
function create()
{
    ...
    stage.stageSprites["rain"] = rain;
    stage.stageSprites["rain2"] = rain2;

    for (char in [boyfriend, dad])
    {
        var shader = new CustomShader("wrath");
        shader.uDirection = 45;
        shader.uOverlayOpacity = 0.5;
        shader.uDistance = 30;
        shader.uChoke = 20;
        shader.uPower = char == dad ? 0.2 : 0.7;

        shader.uShadeColor = [219 / 255, 106 / 255, 104 / 255];
        shader.uOverlayColor = [41 / 255, 8 / 255, 33 / 255];

        var uv = char.frame.uv;
        shader.applyRect = [uv.x, uv.y, uv.width, uv.height];

        if (FlxG.save.data.wrath)
            char.shader = shader;
    }

    heatWaveShader = new CustomShader("heatwave");
    heatWaveShader.time = 0;
    heatWaveShader.speed = 10;
    heatWaveShader.strength = 1.38;

    if (FlxG.save.data.heatwave)
        rain.shader = heatWaveShader;
...
function initMisses()
{
    ...
    missSubtitleText = new FunkinText(0, 0, 0, "Resist.", 70);
    missSubtitleText.addFormat(new FlxTextFormat(0xFFFFFFFF));
    ...
    missesLeftText = new FunkinText(
        missSubtitleText.x,
        missSubtitleText.y + 100,
        0,
        "MISSES LEFT: " + maxMisses,
        40
    );
    missesLeftText.addFormat(new FlxTextFormat(0xFF9B8686));
...
{
    if (missesLeftText == null || missSubtitleText == null)
        return;

    missesLeftText.text =
        "MISSES LEFT: " + (maxMisses - (misses != null ? misses : 0));
    ...
    new FlxTimer().start(intro ? 7.3 : 1.5, function(_)
    {
...
function onPlayerMiss(event)
{
    ...
    var totalMisses:Int =
        PlayState.instance.misses + event.misses;
...
function createHealthbar()
{
    ...
    gorefieldhealthBarBG.loadGraphic(
        Paths.image("game/healthbar/CONVENCIMIENTO_BAR")
    );
    ...
    gorefieldhealthBar.createImageBar(
        Paths.image("game/healthbar/C_BAR"),
        Paths.image("game/healthbar/C_NO_BAR")
    );
    ...
    for (i in 0...2)
    {
        var icon = createIcon(i == 1 ? boyfriend : dad);
        icon.x = 1181;
