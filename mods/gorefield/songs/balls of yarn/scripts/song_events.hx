...
function postCreate() 
{
    ...
    camFollow.setPosition(1480, 850);
    snapCam();
            ...
            if (char.curCharacter == "jon-cc") 
            {
                ...
                if (useMiddleScroll)
                {
                    for (i in 0...strum.members.length) 
                    {
                        ...
                        note.x = ((FlxG.width / 2) - (Note.swagWidth * 2)) + (Note.swagWidth * i);
                    }
                } 
                else 
                {
    ...
    // Create hidden preloaded icons for custom characters
    for (strum in strumLines) 
    {
        for (char in strum.characters)
        {
...
function stepHit(step:Int) {
    switch(step) {
        case 0:
            ...
            FlxTween.tween(FlxG.camera, {zoom: 1}, (Conductor.stepCrochet / 1000) * 127, {
        ...
        case 1536:
            ...
            FlxTween.tween(
                stage.stageSprites["black"],
                {alpha: 1},
                (Conductor.stepCrochet / 1000) * 24,
                {
                    ease: FlxEase.cubeOut,
                    startDelay: (Conductor.stepCrochet / 1000) * 4
                }
            );
    }
}