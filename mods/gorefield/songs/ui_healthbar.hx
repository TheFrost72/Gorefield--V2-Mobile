...
function postCreate() {
    ...
    // Only create icons if the song is not "captive"
    if (PlayState.SONG.meta.name.toLowerCase() != "captive") {
        for (newIcon in 0...2) {
...
static function updateIcons() {
    // Prevent errors if health bar is disabled or icons don't exist
    if (healthBardisabled || gorefieldiconP1 == null || gorefieldiconP2 == null) {
        if (gorefieldiconP1 != null) gorefieldiconP1.visible = false;
        if (gorefieldiconP2 != null) gorefieldiconP2.visible = false;
        return;
    }
...
static function createIcon(character:Character):FlxSprite {
    ...
    if ((character != null && character.xml != null && character.xml.exists("animatedIcon")) ? (character.xml.get("animatedIcon") == "true") : false) {
        ...
        if (icon.graphic != null) {
            ...
            icon.visible = false; // hide icon if graphic doesn't exist
        }
    }

    // These lines always run, independent of whether graphic exists
